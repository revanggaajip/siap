            ...
            <div class="table-responsive">
                <table class="table table-striped table-bordered dt-responsive nowrap" id="dataTable">
                    <thead class="table-dark">
                        <tr>
                            <th width="15">No</th>
                            <th>Nama Kategori Non Medis</th>
                            <th width="17%">Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php foreach($listKNM as $key => $knm) :?>
                        <tr>
                            <td><?= $key+1; ?></td>
                            <td><?= $knm['nama']; ?></td>
                            <td>
                                ...
                                <button type="button" class="btn btn-warning btn-sm text-white"
                                    data-coreui-toggle="modal" data-coreui-target="#editData_<?= $knm['id']; ?>">
                                    <i class="fas fa-edit"></i>&nbsp;Ubah
                                </button>
                                <!-- Akhir Tombol Edit -->

                                <!-- Modal Edit -->
                                <div class="modal fade" id="editData_<?= $knm['id'] ?>" data-coreui-backdrop="static"
                                    data-coreui-keyboard="false" tabindex="-1" aria-labelledby="editModalLabel"
                                    aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title" id="editModalLabel">Form Ubah kategori Non Medis
                                                    <?= $knm['nama']; ?></h5>
                                                <button type="button" class="btn-close" data-coreui-dismiss="modal"
                                                    aria-label="Close"></button>
                                            </div>
                                            <form action="<?= route_to('knm.update',$knm['id']); ?>" method="post">
                                                <?php csrf_field(); ?>
                                                <input type="hidden" name="_method" value="PUT">
                                                <div class="modal-body">
                                                    <div class="mb-2">
                                                        <label for="namakategoriEdit" clask s="form-label">Nama
                                                            kategori Non Medis</label>
                                                        <input type="text" class="form-control" name="nama"
                                                            id="namakategoriEdit" value="<?= $knm['nama']; ?>">
                                                    </div>
                                                </div>
                                ...
                                <button type="button" class="btn btn-danger btn-sm text-white"
                                    data-coreui-toggle="modal" data-coreui-target="#hapusData_<?= $knm['id']; ?>">
                                    <i class="fas fa-trash"></i>&nbsp;Hapus
                                </button>
                                <!-- Akhir Tombol Hapus -->

                                <!-- Modal hapus -->
                                <div class="modal fade" id="hapusData_<?= $knm['id'] ?>" data-coreui-backdrop="static"
                                    ...
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                Apakah anda yakin akan menghapus data <?= $knm['nama']; ?>
                                            </div>
                                            <div class="modal-footer">
                                                <form action="<?= route_to('knm.delete', $knm['id']) ?>" method="post">
                                                    <?php csrf_field(); ?>
